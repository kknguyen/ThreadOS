	...
	case INTERRUPT_SOFTWARE: // System calls
	    ...
	    case OPEN: 
			if ((myTcb = scheduler.getMyTcb()) != null) {
				...
				FileTableEntry ent = filesys.open(s[0], s[1]);
	    ...
	    case SEEK:  
			if ((myTcb = scheduler.getMyTcb()) != null) {
				FileTableEntry ent = myTcb.getFtEnt(param);
				int[] i = (int[])args;
				return filesys.seek(ent, i[0], i[1]);
			} 
			else {
				return ERROR;
			}
	    case FORMAT: 
			return filesys.format(param);
	    case DELETE:
			return filesys.delete((String)args);
	    }
	    return ERROR;
